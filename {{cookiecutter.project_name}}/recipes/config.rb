...
configs = [node['{{ cookiecutter._cookbook_name }}']['config'].keys()].flatten

configs.each do |cfg|
    template ref.call(cfg) do
        # required template attributes
        path                node['{{ cookiecutter._cookbook_name }}']['service'][cfg]['path']
        source              node['{{ cookiecutter._cookbook_name }}']['service'][cfg]['source']
...
ruby_block "config-options" do
    block do
        templates = [node['{{ cookiecutter._cookbook_name }}']['config'].keys()].flatten
        templates.each do |tpl|
            optional_attributes = node['{{ cookiecutter._cookbook_name }}']['config'][tpl].keys() - ['path', 'source']
            template_ref = run_context.resource_collection.find(:template => ref.call(tpl))
            optional_attributes.each do |attr|
                attribute_value = node['{{ cookiecutter._cookbook_name }}']['config'][tpl][attr]
