...
packages = [node['{{ cookiecutter._cookbook_name }}']['package'].keys()].flatten
services = [node['{{ cookiecutter._cookbook_name }}']['service'].keys()].flatten

packages.each do |pkg|
    package ref.call(pkg) do
        # required package attributes
        package_name      node['{{ cookiecutter._cookbook_name }}']['package'][pkg]['package_name']
        action            node['{{ cookiecutter._cookbook_name }}']['package'][pkg]['action']
...
ruby_block "package-options" do 
    block do
        packages = [node['{{ cookiecutter._cookbook_name }}']['package'].keys()].flatten
        packages.each do |pkg|
            optional_attributes = node['{{ cookiecutter._cookbook_name }}']['package'][pkg].keys() - ['package_name', 'action']
            package_ref = run_context.resource_collection.find(:package => ref.call(pkg))
            optional_attributes.each do |attr|
                attribute_value = node['{{ cookiecutter._cookbook_name }}']['package'][pkg][attr]
